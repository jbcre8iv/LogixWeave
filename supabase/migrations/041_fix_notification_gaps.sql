-- Migration: Fix notification gaps for share_revoked, file_parse_failed, permission_changed
-- These actions were logged to project_activity_log but never triggered notifications.

-- 1. Update the notify_project_collaborators() function with new action cases
--    and special handling for share_revoked (revoked user no longer in project_shares)
...
-- 2. Recreate the trigger with expanded WHEN clause
