-- Migration: Add member_left notification support
-- When a collaborator leaves a shared project, notify the owner and other collaborators.

-- 1. Update the notify_project_collaborators() function with member_left case
...
-- 2. Recreate the trigger with member_left added to the WHEN clause
...
CREATE TRIGGER on_project_activity_notify
  ...
  WHEN (NEW.action IN (
    ...
    'share_revoked', 'file_parse_failed', 'permission_changed', 'member_left'
  ))
